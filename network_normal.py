...
from init_variables import weight_variable, bias_variable
...
def decoder(z, dim_latent, activation, width, height, nr_channels, dim_hidden=500):
    ...
    w_log_sigma = weight_variable('w_sigma', [dim_hidden, dim_input])
    b_log_sigma = bias_variable([dim_input])

    return tf.matmul(h2, w_mu) + b_mu, tf.matmul(h2, w_log_sigma) + b_log_sigma
